...
export function gameReducer(state: void | Game, action: GameAction): Game {
  ...
  if (offset > 0) {
    // XXX: Normally this should throw, because the game state will be out of
    // sync from this point on. But this is commented for now as it floods the
    // logs until we can fix the problem holistically. We need some sort of
    // "reverse backfill" as detached game actions can be easily generated by
    // slightly disconnecting from internet whilst playing. Locally actions will
