...
export function gameReducer(state: void | Game, action: GameAction): Game {
  ...
  switch (action.type) {
    ...
    case 'DROP': {
      ...
      if (hasLines(grid)) {
        return clearPlayerLines(state, userId);
      }
      ...
      newState = clearPlayerLines(newState, userId);
      ...
      return sendClearedBlocksToEnemy(newState, userId, grid);
    }
...
function clearPlayerLines(game: Game, userId: UserId): Game {
  const { dropFrames } = game;
  const player = getPlayer(game, userId);
  const { score, lines, grid, dropAcceleration } = player;
  let newGame = game;
...
function sendClearedBlocksToEnemy(
  ...
  const player = getPlayer(game, userId);
  const { blocksCleared } = player;
  const enemy = getEnemyPlayer(game, userId);

  if (!enemy || !blocksCleared.length) {
    return game;
  }

  const rows = unclearedGrid.length;
  const rowsCleared = blocksCleared.map(
    (r, rowIndex) => rows - blocksCleared.length + rowIndex
  );
